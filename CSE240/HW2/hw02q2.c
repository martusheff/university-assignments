...
#define polyMacro(a, b) ({typeof(a) a1=a; typeof(b) b1 = b; a1*a1 + 8 * a1 + 3 *a1*b1 - b1*b1;})
...
void part1(int x) {
	...
	printf("Explanation: it's because x (or 10) was pre-incremented when the isPositive was called.\n\n\n"); // (3 points)
}
...
void part2(int x, int y) {
	...
	printf("In VS : the result of polyFunc = -100 and polyMacro = -100 \n"); 			// (6 points)
	printf("In GCC: the result of polyFunc = -100 and polyMacro = -68 \n\n"); 		// (6 points)
	...
	printf("Explanation: Different compilers evaluate input slightly differently. When using macros or functions, its important to be cautios when using ++ or -- operators.\n"); //  (3 points)
}
