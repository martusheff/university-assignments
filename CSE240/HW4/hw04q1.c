...
int comperator(const void* p, const void* q) {				// comparison function for sorting algorithm
	return strcmp(((struct studentRecord*)p)->studentName,	
				((struct studentRecord*)q)->studentName);	
}
...
int addSort(char* studentName_input, char* major_input, char* schoolYear_input, unsigned int studentID_input)
{
	...
	for(int i = 0; i < count; i++) {	// Checks if list is empty
		if(strcmp(list[i].studentName, studentName_input) == 0)
			return 0;
	}

	strcpy(list[count].studentName, studentName_input);	// Sets current student name
	strcpy(list[count].major, major_input);				// Sets current student major

	if(strcmp(schoolYear_input, "freshman") == 0){		// If-else blose sets current student schoolYear
		...
		printf("Sorry, but %s is not a valid school year input.", schoolYear_input);
	}
	
	list[count].studentID = studentID_input;			// Sets current student ID

	
	
	count++;	// updates count

	qsort(list, count, sizeof(struct studentRecord), comperator);	// sort function (calls comperator)
...
void display()
{
	...
	for(int i = 0; i < count; i++) {							// iterates through list (with count)
		printf("\nStudent name: %s\n", list[i].studentName);	// prints name to screen
		printf("Student Major: %s\n", list[i].major);			// print major to screen

		if(list[i].schoolYear == 0) {							// prints school year to screen
			printf("School Year: Freshman\n");
		} else if((int)list[i].schoolYear == 1) {
			printf("School Year: Sophomore\n");
		} else if((int)list[i].schoolYear == 2) {
			printf("School Year: Junior\n");
		} else {
			printf("School Year: Senior\n");
		}

		printf("Student ID: %d\n", list[i].studentID);			// prints student id to screen
	}
}
...
{
	FILE* file;						// new instance of a file
	file = fopen(fileName, "rb"); // opens file in read binary mode
